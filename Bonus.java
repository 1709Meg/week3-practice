...
public class Bonus {
    ...
    public static double getBonus(int numUnits) {
        if (numUnits >= SALES_UNIT_BONUS) {
            return (SALES_UNIT_REG * numUnits) + (SALES_UNIT_EXTRA * numUnits);
        } else if (numUnits < SALES_UNIT_BONUS && numUnits > 0) {
            return SALES_UNIT_REG * numUnits;
        } else {
            return 0;
        }
    }

    public static double getBonus(int numParts, int bonusLimit) {
        if (numParts > bonusLimit) {
            return numParts * PARTS_BONUS;
        } else {
            return 0;
        }
    }
